"""אונבורדינג מרוכז בשיחה ראשונה (בקשת אלון #6): מגע ראשון אי-פעם → גבר מציג את
עצמו קצר ואוסף פעם אחת שם מלא ומייל; משתמש עם פרופיל/היסטוריה לא מקבל את זה;
בקשת הזמנה מיידית בשיחה ראשונה לא נחסמת — הפרטים נאספים לפני הריצה והתשובה
מושחלת ישירות ל-job. מסלולי MISSING:name/email הקיימים נשארים רשת ביטחון."""
...
def _wire(monkeypatch, converse_result, *, profile=None):
    ...
    return sent, booked, upserts


def test_first_contact_sends_intro(monkeypatch):
    """שיחה ראשונה אי-פעם: הודעת היכרות (גבר + שם + מייל) לפני תשובת השיחה."""
    _reset()
    sent, _, _ = _wire(monkeypatch, {"reply": "אהלן", "ready": False})
    asyncio.run(pipeline.handle_inbound("new1", "היי"))
    assert "גבר" in sent[0] and "שם" in sent[0] and "מייל" in sent[0]
    assert sent[-1] == "אהלן"  # תשובת השיחה עדיין נשלחת אחרי ההיכרות


def test_intro_only_once(monkeypatch):
    """ההיכרות היא חד-פעמית — הודעה שנייה באותה שיחה לא מקבלת אותה שוב."""
    _reset()
    sent, _, _ = _wire(monkeypatch, {"reply": "אהלן", "ready": False})
...
def test_known_profile_skips_intro(monkeypatch):
    ...
    sent, _, _ = _wire(monkeypatch, {"reply": "אהלן", "ready": False}, profile=prof)
...
def test_first_conversation_booking_gates_then_runs(monkeypatch):
    """בקשת הזמנה מיידית בשיחה ראשונה: לא נחסמת ולא רצה בלי זהות — נאספים שם+מייל
    פעם אחת (ההיכרות כבר ביקשה, בלי שאלה כפולה), והתשובה מושחלת ישירות לריצה."""
    _reset()
    ready = {
        "reply": "על זה",
        "ready": True,
    ...
    sent, booked, upserts = _wire(monkeypatch, ready)
    ...
    asyncio.run(go())
    assert booked == []  # הריצה מחכה לפרטים
    assert pipeline._await_answer["new3"]["field"] == "contact"
    assert pipeline._booking["new3"]["state"] == "missing"
    assert sum("מייל" in m for m in sent) == 1  # ההיכרות שאלה — אין שאלת המשך כפולה
    ...
    assert upserts and upserts[-1] == {"name": "אלון בזק", "email": "alon@example.com"}
...
def test_contact_answer_in_two_natural_steps(monkeypatch):
    ...
    _reset()
    sent, booked, upserts = _wire(monkeypatch, {"reply": "רגע", "ready": False})
...
def test_profile_user_ready_runs_without_gate(monkeypatch):
    ...
    sent, booked, _ = _wire(monkeypatch, ready, profile=prof)
...
def test_gate_asks_when_not_first_contact(monkeypatch):
    ...
    sent, booked, _ = _wire(monkeypatch, ready)
