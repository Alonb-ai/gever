...
def test_failure_reply_events_wordings_and_restaurant_unchanged():
    ...
    info, msg = pipeline._failure_reply("sold_out", "קובי פרץ", task_type="events")
    ...
    assert pipeline._failure_reply("sold_out", "גרקו") is None
    assert pipeline._failure_reply("no_event_in_city", "גרקו") is None
    # ומסעדה רגילה לא זזה
