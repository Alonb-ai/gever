...
def test_cinema_card_wall_link_is_live_view_when_session_alive(monkeypatch):
    ...
    נוחת על מסך הכרטיס עם כל מה שכבר מולא), לא לינק הדף הרגיל. מאז דפדפן-גבר
    (main) הלינק ממותג /b/{token} — ה-URL של browserbase לא נחשף, וה-token
    פותר חזרה ל-Live View האמיתי."""
    _reset()
    ...
    assert pipeline._booking["c10"]["info"] == f"{settings.public_base_url}/b/{token}"
