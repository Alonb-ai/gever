...
def _timeout_s(job: dict) -> int:
    """תקרת הריצה לפי סוג המשימה — קולנוע/הופעות מקבלים את התקרה הארוכה (מפת
    אולם/מושבים + טפסים), וביטוח את הארוכה מכולן (טופס רב-דפים)."""
    task_type = job.get("task_type")
    if task_type == "insurance":
        return BU_INSURANCE_TIMEOUT_S
    return BU_CINEMA_TIMEOUT_S if task_type in ("cinema", "events") else BU_TIMEOUT_S
...
def _max_steps(task_type: str) -> int:
    ...
    מרובי-מסכים (קולנוע/הופעות) נשארים על 40; ביטוח — טופס רב-דפים עתיר שדות: 80."""
    if task_type == "insurance":
        return 80
...
async def book_table_bu(
    *,
    ...
    task_type: str = "restaurant",  # restaurant | cinema | events | insurance — בורר את ה-task
