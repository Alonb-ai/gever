...
דו-שלבי (החלטת אלון, 17.7 — אחרי שהבנצ' הלילי הראה נחיתות-יעד שגויות דטרמיניסטיות):
שלב 1 — חיפוש פנימי בפלטפורמות עצמן (ה-autocomplete של Ontopo וה-bridge של Tabit):
שמות קנוניים, סניפים אמיתיים, ו-aliases עברית↔אנגלית. endpoints לא רשמיים → כל
כשל נופל בשקט לשלב 2. שלב 2 (fallback) — חיפוש web ב-Brave בשאילתה רחבה
("<שם> הזמנת מקום") ודיסאמביגואציה לפי כותרת. בשני השלבים: התאמה דורשת את מילת
המותג (טוקן לא-גנרי ראשון), טוקן סניף מכריע בין סניפים, ודף-רפאים של Ontopo נפסל.
