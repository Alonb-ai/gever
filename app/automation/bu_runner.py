...
- ברשת הוט סינמה ההזמנה עוברת לדומיין נפרד tickets.hotcinema.co.il (אשף:
  ...
  של הוט כל מושב הוא svg מקונן עם class ‏"available status" — מדוד bbox של שני
  מושבים צמודים באמצע ולחץ נטיבית על מרכז כל אחד (אומת חי 19.7)."""
...
- מפת המושבים היא לרוב SVG: קליק שלא נתפס — אל תנחש selectors בניסיונות עיוורים;
  ...
  לחיצת JS דרך evaluate היא אירוע לא-אמיתי שאתרים מודרניים מתעלמים ממנו — evaluate
  משמש למדידה בלבד. שים לב: מושבים הם לרוב *לא* rect/path — חפש רחב: svg מקונן בתוך
  svg, אלמנטי image/use, ו-custom elements, עם class של זמינות (למשל svg.available).
  מצאת את אלמנטי המושבים — שלוף getBoundingClientRect, ואז בצע click עם
  coordinate_x/coordinate_y על מרכז ה-bbox בדיוק כפי שנמדד (אלו קואורדינטות סופיות —
  אל תתאים ואל תעגל לרשת אחרת), לא על ניחוש מצילום המסך. זום או גלילה מזיזים הכל —
  מדוד מחדש מיד לפני הקליק.
