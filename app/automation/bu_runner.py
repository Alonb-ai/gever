...
- הצהרת הבריאות: תשובות הלקוח — "{ins.get("health") or "אין"}". "אין" פירושו שהלקוח ענה
  ...
  מותר לסמן "לא". כל שאלה בריאותית אחרת, או שאלה שהתשובות שבידך לא עונות עליה
  חד-משמעית — זו הצהרה משפטית: לעולם אל תענה בשם הלקוח, עצור עם MISSING ושורת FIELD
  שמצטטת את נוסח השאלה מהדף.
