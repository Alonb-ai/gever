...
# נוסתה ונמחקה (22.7): הנחיה בפרומפט "אל תוסיף wait ידני" לא הפחיתה כלום — ריצות
# שנשאו אותה עדיין הוציאו 1-2 צעדי wait. הפתרון הוא לא פרומפט אלא poll התייצבות
# לפני דגימת ה-state (לבר #3 בדוח הזירוזים 22.7).
...
התחל מהכתובת: {job["url"]} (דף הסרט באתר הרשת)."""
    ...
    hot = (
        """
- בדף הסרט של הוט לוח ההקרנות נפתח על תאריך היום; כדי לעבור לתאריך המבוקש לחץ על
  שדה התאריך — ייפתח לוח-שנה — ובחר בו את היום. אל תבזבז צעדים על הקלדה או evaluate
  על שדה התאריך: הוא יושב ב-shadow DOM מקונן וקלט טקסט/סקריפט לא נקלט בו (אומת חי 21.7).
- ברשת הוט סינמה ההזמנה עוברת לדומיין נפרד tickets.hotcinema.co.il (אשף:
  ...
  השלבים. קיר-התשלום (טופס פרטים + אשראי) הוא נקודת העצירה הרגילה. במפת המושבים
  של הוט כל מושב הוא svg מקונן עם class ‏"available status" והציור הוא <image>:
  ב-evaluate אחד — מצא שני מושבים צמודים באמצע (querySelectorAll על
  svg.available.status), קח מרכז bbox, שלוף elementFromPoint ושגר עליו את רצף
  האירועים המלא, ואמת שהמונה "מתוך" עלה (המתכון המוכח חי 21.7 — קליק עכבר אמיתי
  נבלע בשכבת הגרירה של המפה)."""
...
- בחר את בית הקולנוע לפי העיר/הסניף שהלקוח ביקש ({job["city"]}). אין לרשת סניף
  ...
  ראית בדף. אל תסיק "אין סניף" מ-dropdown ניווט כללי של האתר — רשימות סניפים כאלה
  עשויות להיות חלקיות; קבע רק לפי לוח ההקרנות של הסרט עצמו בתאריך המבוקש.
...
- מפת המושבים היא לרוב SVG: קליק שלא נתפס — אל תנחש selectors בניסיונות עיוורים;
  ...
  שים לב: מושבים הם לרוב *לא* rect/path — חפש רחב: svg מקונן בתוך svg, אלמנטי
  image/use ו-custom elements, עם class של זמינות (למשל svg.available). המתכון
  שהוכח חי (19-21.7): מדוד getBoundingClientRect של המושב, ובאותו evaluate עצמו
  שלוף elementFromPoint(מרכז ה-bbox) ושגר עליו רצף מלא
  pointerdown/mousedown/pointerup/mouseup/click עם clientX/Y — למפות עם שכבת
  גרירה/זום זה עוקף אותה, בעוד קליק עכבר אמיתי נבלע בה. מיד אחרי — קרא את מונה
  הבחירה מהדף ואמת שעלה. לא עלה אחרי שני ניסיונות → נסה click עם
  coordinate_x/coordinate_y על אותן קואורדינטות בדיוק (בלי עיגול); זום/גלילה
  מזיזים הכל — מדוד מחדש לפני כל קליק. גם זה לא — עצור בכנות.
- מודאל/הודעה שקופצים באמצע (למשל "מכירת הכרטיסים לאירוע הסתיימה", "אין מספיק
  מושבים") הם תשובה עסקית, לא תקלה — קרא אותם ודווח בכנות (FAILED מתאים או
  MISSING עם החלופות), אל תילחם בדף.
...
- המעבר משלב לשלב (ובמיוחד לתשלום) עשוי לכלול אימות אבטחה אוטומטי — כפתור שלא
  ...
  לאימות, ונסה שוב פעם אחת לפני שאתה משנה גישה. אימות שדורש פתרון אנושי
  (reCAPTCHA/זיהוי תמונות) אחרי שהמושבים כבר נבחרו — אל תנסה לפתור ואל תדווח
  כישלון: זה קיר-הכרטיס שלך. עצור ודווח CARD_REQUIRED עם הסיכום הרגיל וציין
  שנדרש אימות — הלקוח יפתור אותו בעצמו בדפדפן שלו יחד עם התשלום.{hot}
{_NAV_GENERIC}
{_STEP_DIET}
...
- המעבר משלב לשלב (ובמיוחד לתשלום) עשוי לכלול אימות אבטחה אוטומטי — כפתור שלא
  ...
  (reCAPTCHA/זיהוי תמונות) אחרי שהמושבים כבר נבחרו — אל תנסה לפתור ואל תדווח
  כישלון: זה קיר-הכרטיס שלך. עצור ודווח CARD_REQUIRED עם הסיכום הרגיל וציין
  שנדרש אימות — הלקוח יפתור אותו בעצמו בדפדפן שלו יחד עם התשלום.
{_NAV_GENERIC}
{_STEP_DIET}
...
- הצעות מכירה-צולבת ושיווק שאינן חלק מהצעת ביטוח הנסיעות — ביטוח רכב/דירה, שובר או
  מתנה תמורת פרטים, הרשמה לדיוור — אינן שדה חובה של ההצעה: בחר את אפשרות הסירוב/הדילוג
  ("אוותר"/"לא תודה") והמשך. אל תעצור עליהן עם MISSING ואל תמסור פרטים עבורן.
- התקדם שלב-שלב עד *הצעת המחיר* — המסך שמציג את הפרמיה לתשלום עבור הנסיעה. זהו
  מסך הסיכום שלך.
- מסך התשלום אינו "שדות חסרים": כשהגעת למסך שמבקש פרטי כרטיס אשראי — שם בעל הכרטיס,
  מספר כרטיס, ת"ז של בעל הכרטיס, תוקף או CVV, או הצהרת "אני בעל הכרטיס" — זהו קיר
  התשלום שאחרי הצעת המחיר, לא שדות חובה חסרים. לעולם אל תדווח עליהם MISSING ולעולם אל
  תבקש אותם מהלקוח. זהו מסך הסיכום שלך: דווח SUMMARY_REACHED עם הפרמיה ואז CARD_REQUIRED.
