...
def _notes_line(job: dict) -> str:
    ...
    return f'\nהעדפות מהלקוח: "{n}" — כבד אותן בבחירות שהאתר מציע.' if n else ""
...
- בחר את מספר הסועדים, התאריך והשעה המבוקשים. השעה המבוקשת קודמת לכל: אם היא
  ...
  רק אם המבוקשת באמת לא מופיעה או מסומנת תפוסה — הזמינה הקרובה ביותר, עד 30
  דקות הפרש לכל היותר (קדימה או אחורה). אין זמינות בטווח → FAILED:no_availability.
