...
_EXTRACT = (
    ...
    "איתו אחת במדויק). בקשת הזמנה מפורשת שכוללת את כל הפרטים ('שולחן לשניים "
    "בראשון ב-16:00 ב-X') היא עצמה האישור — סמן ready=true מיד, אל תבקש אישור "
    "נוסף ואל תחכה לכלום. "
    "'confirm'=true רק כשכבר הגעתם למסך האישור (יש הזמנה ממתינה) והמשתמש מאשר "
    "במפורש לסגור אותה סופית — לא להתחלה של הזמנה חדשה. "
    "עקביות בין דיבור למעשה: אם ב-reply אתה אומר שאתה על זה / בודק / יוצא לסגור — "
    "אתה חייב לסמן ready=true (או confirm=true) באותה תשובה בדיוק; הבטחה בלי דגל = "
    "שום דבר לא קורה והלקוח מחכה לכלום. חסר לך פרט כדי לסמן ready? אל תבטיח — שאל. "
