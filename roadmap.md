...
## הערות מ-Dry Run (יוני 2026) — לתעד, טרם טופל
ממצאים מהרצה חיה ב-WhatsApp. רק לתיעוד — לא לתיקון כעת.
- [ ] גבר עונה "קר" ולא מזמין/חם (יש צילום מסך). [זרוע 2 — persona]
- [ ] ה"מטפל בזה" אורך הרבה זמן בלי חיווי למשתמש → תחושה שנתקע/מת. צריך למלא: שיחת חולין / עדכוני סטטוס תוך כדי / הודעת "תמשיך בשלך, אעדכן כשסגור". [זרוע 1+3 — UX בזמן ריצה ארוכה]
- [ ] הריצה אורכת מעל 5 דקות לסגירת שולחן. [זרוע 1 — ביצוע איטי]
- [ ] צריך תמיכה גם בהודעות קוליות (ElevenLabs או חלופה). [חדש — קלט/פלט קולי]
- [ ] אחרי 8 דקות אין תגובה — נראה שההליך נתקע. סיכום ה-dry-run: המודל נתקע ולא סגר מסעדה כמבוקש. [זרוע 1 — באג תקיעה, לחקור (עדיפות גבוהה)]
...
- [ ] `poc/gever.py` חולק לוגיקת שיחה עם `app/pipeline.py` — איחוד נדחה (כדי לא לשבור את ארגומנט המגדר ב-gever.py)
